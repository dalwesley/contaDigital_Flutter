...
class HomeUpdatePassword extends StatefulWidget {
  // final UserModel userModel;
  // UserEditor({@required this.userModel});

  HomeUpdatePassword({Key key, @required this.cardNumber}) : super(key: key);
  final cardNumber;

  @override
  _HomeUpdatePasswordState createState() => _HomeUpdatePasswordState();
}

class _HomeUpdatePasswordState extends State<HomeUpdatePassword> {
  ...
  handleCardDetails(cardNumber) {
    ...
    cardBloc.details(cardNumber).then((data) {}).catchError((err) {
      onError();
    }).whenComplete(() {
      onComplete();
    });
  ...
  void goToTerm() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => HomeTermView(comingFrom: 'password'),
      ),
    );
  }
